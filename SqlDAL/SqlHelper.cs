...
namespace SqlDAL
{
    ...
    public sealed class SqlHelper
    {
        ...
        #region 数据库连接
        /// <summary> 
        /// 一个有效的数据库连接字符串，默认返回master数据库 
        /// </summary> 
        /// <returns></returns> 
        public static string GetConnString(DBEnum dbkey=DBEnum.ATP_MASTER)
        {
            var manager = DBConfig.ResourceManager;
            return manager.GetString(dbkey.ToString());
        }
        /// <summary> 
        /// 一个有效的数据库连接对象 
        /// </summary> 
        ...
        public static SqlConnection GetConnection(string ConnStr="")
        {
            if(ConnStr.Length>0)
                ...
                return new SqlConnection(SqlHelper.GetConnString());
        }
        #endregion
        #region 执行多条sql语句

        public static bool ExecuteBatch(IList<string> cmdStrList, IList<SqlParameter[]> paraArrList)
        {
            if (cmdStrList == null || paraArrList==null)
            {
                throw new ArgumentNullException("命令与参数不能为Null");
            }
            
            if (cmdStrList.Count != paraArrList.Count)
            {
                throw new ArgumentException("命令或参数表数量不一致，命令数：" + cmdStrList.Count + ",参数:" + paraArrList.Count);
            }
            var conn = GetConnection();
            var tran = conn.BeginTransaction();
            try
            {
                for (var i = 0; i < cmdStrList.Count; i++)
                {
                    var cmd = new SqlCommand();
                    var close = false;
                    PrepareCommand(cmd, conn, tran, CommandType.Text, cmdStrList[i], paraArrList[i], out close);
                    cmd.ExecuteNonQuery();
                    if (close)
                        conn.Close();
                }
